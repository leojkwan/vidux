#!/usr/bin/env bash
# vidux-burst.sh — Burst mode: drain the queue, don't stop until empty.
#
# The structural counterpart of vidux-loop.sh (watch mode).
# - Watch mode (vidux-loop.sh): <2 min, read state, decide whether to fire burst.
# - Burst mode (this script):   no time limit, keep executing until queue drains.
#
# Usage:
#   bash vidux-burst.sh <plan-path>              # Run burst: pop tasks until queue empty
#   bash vidux-burst.sh <plan-path> --dry-run    # Show what would be done, don't execute
#
# Output: JSON with burst results (slices_completed, surprises, next_burst_recommended)
#
...
[[ -z "$PLAN" ]] && { echo '{"error": "Usage: vidux-burst.sh <plan-path> [--dry-run]"}'; exit 1; }
...
if type vidux_emit_loop_start &>/dev/null 2>&1; then
  vidux_emit_loop_start "$PROJECT_NAME" "$PLAN" "burst" 2>/dev/null || true
fi
  ...
  cat <<EOF
{
  ...
  "recommendation": "$([ "$PENDING_AT_START" -gt 0 ] && echo "fire_burst" || echo "nothing_pending")"
}
EOF
  exit 0
fi
...
if type vidux_emit_loop_end &>/dev/null 2>&1; then
  vidux_emit_loop_end "$PROJECT_NAME" "$PLAN" "$ACTION" "burst" "$TASK" 2>/dev/null || true
fi
