...
- [pending] **18.7 Verify fleet recovery after Codex restart** — Check next cycle's memory notes. At least 3/5 should dispatch real work (acme-asc has 9 tasks, acme-web has 6, beacon has 6). [Depends: Codex restart]
...
- [completed] **20.1 Restructure /codex skill into generic core + vidux section** — Move vidux-specific content (Quick Check gate, PLAN.md references, vidux-loop.sh, Safety Mechanisms) into a clearly marked "## Vidux Integration (Optional)" section at the bottom. Generic core covers: DB ops, memory files, prompt 8-block structure, fleet ops, ghost cleanup, SCAN gate (works without vidux), known bugs. Description changes from "Coupled with vidux" to "Standalone fleet management, with optional vidux integration." [Evidence: 13 vidux-coupled lines identified in SKILL.md]
- [completed] **20.2 Add generic gate pattern for non-vidux automations** — Not every automation uses vidux-loop.sh. Add a "Simple Gate" pattern: check memory for 3x same result → exit. No PLAN.md, no vidux-loop.sh. The Quick Check gate becomes vidux-only. SCAN gate already works standalone. [Evidence: Snap use case — automations without plan-first orchestration]
- [completed] **20.3 Rename vidux-watchdog → codex-watch in DB** — Kill app-server, rename automation ID in sqlite, rename memory directory from ~/.codex/automations/vidux-watchdog/ to ~/.codex/automations/codex-watch/. Update prompt to remove "vidux" from mission text. [Depends: 20.1]
- [completed] **20.4 Add `/codex watch` subcommand** — Fleet health scan as a /codex subcommand (not a separate automation). Reads all memories, classifies fleet, outputs scorecard. This is what you run interactively to check fleet health. The codex-watch *automation* runs it on a schedule; `/codex watch` runs it on-demand. [Depends: 20.1]
- [completed] **20.5 Update PLAN.md Progress references** — Change "Fleet watchdog" → "Fleet watch" in 4 Progress entries. Historical accuracy preserved (dates unchanged). [Depends: 20.3]
- [completed] **20.6 Update vidux SKILL.md and DOCTRINE.md** — No watchdog refs existed in SKILL.md or DOCTRINE.md. All fleet health scanning now documented in /codex skill under `/codex watch`. [Done: 2026-04-08] — Remove any "watchdog" references. Where vidux docs mention fleet health scanning, point to `/codex watch` instead. Vidux owns plan orchestration; Codex owns fleet operations. Clean boundary. [Depends: 20.1]
...
<!-- 6 tasks archived to ARCHIVE.md -->
