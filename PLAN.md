...
#### 10.9 — Native Codex docs + worktree-GC follow-up [completed] [Agent: codex/claudux-dogfood]

1. Clarify in the public fleet docs that the TOML + DB flow is the native Codex exception path, while `Chat` remains the default for Codex-created automations in the automation guide.
2. Update the public worktree lifecycle summaries to reflect the current 2-4 lane guidance plus the `vidux-worktree-gc.py` 2.20.0 protection for the invocation checkout.
3. Preserve the current VitePress IA/config and re-run the config-link audit, docs build, and repo test gate.
[Evidence: guides/automation.md:141, docs/fleet/platforms.md:5, scripts/vidux-worktree-gc.py:1, tests/test_worktree_gc.py:143]
[Done: 2026-04-27. Narrow public-docs refresh only: `docs/fleet/{index,platforms,codex-lifecycle,codex-setup,operations}.md` now scope the native Codex path explicitly, and `docs/reference/scripts.md` now documents `merged_clean`-only cleanup plus primary/invocation checkout protection. Gate: config-link audit PASS (31 links), `python3 -m unittest tests.test_vidux_contracts tests.test_plan_gc` PASS (156/156), `npm ci` PASS, `npm run docs:build` PASS.]
...
- [2026-04-27 01:40 EDT] 10.9 completed. Static-analysis follow-up against HEAD `6cba9e7` found only two real drifts after the 2.20.0 worktree release: the public Codex fleet pages still read like every Codex automation uses the native TOML + DB path, and the docs-site worktree summary had not yet called out invocation-checkout protection or the current 2-4 lane guidance. Kept the existing VitePress IA and config intact, patched only the six stale public docs pages, and re-ran config-link audit + contracts + docs build. Peer memory check showed the last `claudux-dogfood` vidux ship was 2026-04-26 22:39 EDT, so there was no live competing writer on this surface.
