...
### Phase 11: Fake-project regression cron [in_progress]

**Goal:** Prove the two highest-risk automation foundations on disposable fixtures: ledger worktree GC safety and vidux plan selection/GC behavior. The recurring lane should be chat-scoped, not a local Codex TOML automation pointed at a worktree.

**Evidence:**
- [Source: Leo request 2026-04-24] "crun cron every 10 min let's run tests with fake projects ensuring ledger GC and vidux planning work as expected"

- [completed] 11.1 Add fake-project regression tests for ledger GC classification/dry-run and vidux plan GC/selection. [Evidence: Leo request 2026-04-24] [Tests: tests/test_fake_project_flows.py]
- [blocked] 11.2 Create the 10-minute chat-scoped cron for the fake-project test suite. [Evidence: Leo request 2026-04-24] [Blocker: chat automation tool is not exposed in this Codex session; local TOML cron rollback complete]
...
- [2026-04-24 00:22 EDT] Phase 11 partial: added `tests/test_fake_project_flows.py` with disposable Vidux plan GC/task-selection coverage plus local ledger worktree GC classify/dry-run coverage. Gates: `python3 -m unittest tests.test_fake_project_flows tests.test_plan_gc -v` passed (19/19), `bash scripts/vidux-loop.sh PLAN.md` emitted valid JSON, and `python3 -m pytest -q` passed (155/155). User corrected scheduler target to chat-scoped cron, not local TOML/worktree automation; the temporary local `vidux-fake-project-gc` TOML, prompt/memory files, and DB row were removed. Blocker: no chat automation creation tool is exposed in this Codex session.
