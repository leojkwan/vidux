...
- [ ] **11.1 Bake dispatch/reduce/self-extending doctrine into DOCTRINE.md** — DOCTRINE.md has 0 mentions of burst, reduce mode, closure bias, or self-extending plans. SKILL.md mentions bimodal (11 refs) but not the structural enforcement. Add Doctrine 10 (dispatch/reduce) and Doctrine 11 (self-extending plans with bounded recursion). [Evidence: grep shows 0 burst refs in DOCTRINE.md, memory feedback_vidux_automation_doctrine.md has the three tenets]
- [ ] **11.2 vidux-dispatch.sh** — New script: long-running deep-work mode. No upper time bound. Drains queue until empty, hard blocker, or context budget. Spawns subagents when context fills. Exits clean only on queue drain. ~150 lines. [Evidence: burst-vs-watch-mode-insight.md architecture] [Depends: 11.1]
- [ ] **11.3 Tighten vidux-loop.sh to explicit reduce mode** — Add hard 2-minute budget. Add `next_action: burst|none` to JSON output. Forbid code changes in reduce mode (read-only assessment). [Evidence: Gastown Witness pattern, burst-vs-watch-mode-insight.md] [Depends: 11.1]
...
- [pending] Task 12.4: Build vidux-witness.sh — lightweight script that runs hourly, reads all automation memories, detects stuck/mid-zone/handoff-gap patterns, and writes a one-line status per automation to a fleet-status.md file. No code changes, read-only. [Evidence: Gastown's Witness role detects stuck workers within 15 minutes]
