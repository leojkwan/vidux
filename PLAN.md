...
**Side effects are agnostic (Leo 2026-04-11):** This plan covers ONLY the cloud-agnostic draft-PR mechanics. Paid-service integrations (Greptile review, Sentry context, Seer fixes, Nia indexing) live outside the core `/vidux` scope as composable skills bolted on after the core works. Core ships independently. Research: `investigations/paid-tooling-pr-integration.md`.
...
2. Update header to note: "This reference contains the full doctrine that used to be split across companion automation docs." Section 19 activation line changed from "activate /vidux-auto alongside /vidux" to "Read this reference (loaded on demand from within a /vidux session)".
...
#### 10.7 — Single-entry `/vidux` references sweep [completed] [Depends: 10.6]

1. Route deep automation doctrine via `references/automation.md` (SKILL.md companion-surface link + Activation rule).
2. Remove active `/vidux-codex` references from Phase 5 copy (replace with "outside core `/vidux` scope").
[Done: 2026-04-23]
...
- [DIRECTION] [2026-04-11] vidux core is open-source and cloud-agnostic. Phase 5 contains ONLY draft-PR mechanics — no Greptile, no Sentry, no Nia, no Seer. Paid-service integrations live outside the core `/vidux` scope (formerly `/vidux-codex`) as composable skills, not in this plan. "Keep side effects like greptile and followups agnostic, that is more a /vidux-codex kinda thing." — Leo.
...
- [2026-04-22 23:45 EDT] Added a contracts gate to prevent `/vidux-auto` from reappearing in active docs/commands: only historical PLAN/CHANGELOG/ARCHIVE and `evidence/` may mention it; `commands/vidux-auto.md` must remain deleted. Gate: `python3 -m pytest -q tests/test_vidux_contracts.py` passes (136/136).
- [2026-04-23T04:51Z] Single-entry `/vidux` sweep: route deep automation doctrine via `references/automation.md` (SKILL.md companion-surface link + Activation rule) and scrub remaining active `/vidux-codex` mentions to "outside core `/vidux` scope" phrasing.
