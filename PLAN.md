...
- [pending] **18.7 Handle find_work in with-vidux gate prompts** — vidux-loop.sh now emits next_action:"find_work" on queue_starved, but resplit-web and strongyes-release-train prompts only check "none" (exit) and "dispatch" (work). Update step 4 to: "If next_action is dispatch or find_work: proceed." [Evidence: fleet watchdog 2026-04-08, resplit-currency memory showing find_work exit]
