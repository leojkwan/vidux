...
`vidux-inbox-sync.py` also refuses unsafe Linear sources before mutation:
team-wide sync with no `project_id` requires `allow_team_wide: true`, and
project-scoped sync without `project_name` requires
`allow_unguarded_project: true`. Repo-owned codebase lanes should not set
either flag.
