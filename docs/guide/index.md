...
**Append-only logs** — the `## Progress` section in `PLAN.md` and each lane's `memory.md` are strictly append-only. Some overlays also keep a separate `PROGRESS.md`, but core vidux does not require it.
